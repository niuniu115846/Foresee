        ...
        if any(filename.lower().endswith(ext) for ext in img_exts):
            ...
            try:
                ...
                if (draw is not None 
                    ...
                    print(f"download mask: {mask_path}")
                else:
                    cv2.imwrite(mask_path, np.zeros(img.shape[:2], dtype=np.uint8))
                    print(f"no point detected: {mask_path}")

            except Exception as e:
                print(f"{filename} fail（{e}），download black mask")
                result_img = img 
                cv2.imwrite(mask_path, np.zeros(img.shape[:2], dtype=np.uint8))

            result_path = os.path.join(output_folder, filename)
            cv2.imwrite(result_path, result_img)
            print(f"download: {result_path}")
